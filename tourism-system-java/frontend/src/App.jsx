...
function App() {
  ...
  if (!isLoggedIn) {
    return <LoginPage onLoginSuccess={(userData) => { login(userData); navigate('/'); showToast('success', '登录成功'); }} />;
  }
  ...
  return (
    <div className="flex min-h-screen bg-white">
      <LeftSidebar currentUser={user} onLogout={handleLogout} className="hidden md:flex" />
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <main className="flex-1 md:ml-64 xl:mr-72 pt-[60px] md:pt-0">
                          ...
                          <dd className="font-sans text-sm text-body">
                            {(user?.interests || []).length > 0
                              ? (user.interests || []).map((item) => (
                                  <span key={item} className="inline-block px-3 py-1 mr-2 text-xs bg-neutral-50 border border-neutral-200">{item}</span>
                                ))
                              : '暂无'}
                          </dd>
                        </div>
      ...
      <RightPanel currentUser={user} onLogout={handleLogout} className="hidden xl:block" />
    </div>
  );
}
