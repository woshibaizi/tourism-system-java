    ...
    <div className="max-w-screen-xl mx-auto px-6 py-12">
        ...
        <div className="flex-1 flex gap-2">
          ...
          <div className="flex-1 flex">
            <input
              ...
              className="flex-1 border border-border px-3 py-2 text-sm font-sans focus:border-neutral-900 focus:ring-0 focus:outline-none"
            />
            <button onClick={handleSearch} className="px-4 bg-heading text-white hover:opacity-90 transition-colors">
              <Search size={16} />
            </button>
          </div>
        </div>
        <div className="flex gap-2">
          {SORT_OPTIONS.map((o) => (
            <button
              ...
              className={`px-4 py-2 text-xs font-sans uppercase tracking-widest border transition-colors ${sortType === o.value ? 'border-heading text-heading' : 'border-border text-muted hover:text-heading'}`}
            >
              {o.label}
            </button>
          ))}
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {paginatedDiaries.map((diary) => (
              <div
                key={diary.id}
                className="group cursor-pointer border border-transparent hover:border-border transition-colors"
              >
                <div className="aspect-[3/2] overflow-hidden bg-accent-soft relative">
            ...
            <div className="flex justify-center gap-2 mt-12">
              {Array.from({ length: totalPages }, (_, i) => (
                <button
                  ...
                  className={`w-10 h-10 font-sans text-sm border ${currentPage === i + 1 ? 'border-black bg-black text-white' : 'border-border hover:border-black text-muted'}`}
                >
                  {i + 1}
                </button>
              ))}
      ...
      {createOpen && (
        <div className="fixed inset-0 z-[200] bg-black/40 flex items-center justify-center" onClick={() => setCreateOpen(false)}>
          <div className="bg-surface w-full max-w-lg mx-4 p-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                  className="w-full border border-border px-3 py-2 text-sm font-sans focus:border-accent focus:ring-0 focus:outline-none" />
              </div>
              <div>
                ...
                <textarea rows={5} value={formData.content} onChange={(e) => setFormData({ ...formData, content: e.target.value })}
                  className="w-full border border-border px-3 py-2 text-sm font-sans focus:border-accent focus:ring-0 focus:outline-none resize-none" />
              </div>
              <div>
