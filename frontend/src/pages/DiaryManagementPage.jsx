...
function DiaryManagementPage() {
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-6 py-12">
      ...
      {/* Filters */}
      <div className="flex flex-col md:flex-row gap-3 mb-6 pb-6 border-b border-border">
        ...
        <select value={selectedPlace} onChange={(e) => setSelectedPlace(e.target.value)} className="border border-border px-3 py-2 text-xs font-sans bg-surface">
          <option value="">全部景区</option>
          {places.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
        </select>
        <select value={selectedAuthor} onChange={(e) => setSelectedAuthor(e.target.value)} className="border border-border px-3 py-2 text-xs font-sans bg-surface">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((d) => (
              <tr key={d.id} className="border-b border-border hover:bg-accent-soft">
      ...
      {editOpen && (
        <div className="fixed inset-0 z-[200] bg-black/40 flex items-center justify-center" onClick={() => setEditOpen(false)}>
          <div className="bg-surface w-full max-w-lg mx-4 p-6" onClick={(e) => e.stopPropagation()}>
