...
function PlaceDetailPage() {
  ...
  return (
    <div>
      {/* Hero */}
      <section className="relative w-full h-[400px] md:h-[500px] bg-neutral-200">
        {placeData.image ? (
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {placeData.keywords.map((kw) => (
                    <span key={kw} className="px-3 py-1 text-xs bg-neutral-50 border border-neutral-200 font-sans">{kw}</span>
                  ))}
                </div>
              </div>
            )}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {placeData.features.map((f) => (
                    <span key={f} className="px-3 py-1 text-xs bg-neutral-50 border border-neutral-200 font-sans">{f}</span>
                  ))}
                </div>
              </div>
            )}
                ...
                {bldPages > 1 && (
                  <div className="flex gap-2 mt-4">
                    {Array.from({ length: bldPages }, (_, i) => (
                      <button key={i} onClick={() => setBldPage(i + 1)} className={`w-8 h-8 text-xs border ${bldPage === i + 1 ? 'bg-black text-white border-black' : 'border-neutral-200'}`}>{i + 1}</button>
                    ))}
                  </div>
                )}
              </div>
            ...
            {facilities.length > 0 && (
              <div>
                ...
                <div className="space-y-3">
                  {paginatedFac.map((f) => (
                    <div key={f.id} className="border border-neutral-100 p-4">
                ...
                {facPages > 1 && (
                  <div className="flex gap-2 mt-4">
                    {Array.from({ length: facPages }, (_, i) => (
                      <button key={i} onClick={() => setFacPage(i + 1)} className={`w-8 h-8 text-xs border ${facPage === i + 1 ? 'bg-black text-white border-black' : 'border-neutral-200'}`}>{i + 1}</button>
                    ))}
                  </div>
                )}
              </div>
          ...
          {/* Sidebar */}
          <div>
            <div className="border border-neutral-100 p-6 sticky top-8 space-y-6">
              <div>
