...
import { Search, MapPin, Navigation, Car, Footprints, Bike, ExternalLink, Share2, Check } from 'lucide-react';
...
function RoutePage() {
  ...
  const openInExternalMap = () => {
    if (!selectedStart || !selectedEnd) return;
    const slng = selectedStart.location?.lng ?? selectedStart.location?.getLng?.();
    const slat = selectedStart.location?.lat ?? selectedStart.location?.getLat?.();
    const elng = selectedEnd.location?.lng ?? selectedEnd.location?.getLng?.();
    const elat = selectedEnd.location?.lat ?? selectedEnd.location?.getLat?.();
    const url = `https://uri.amap.com/navigation?from=${slng},${slat},${encodeURIComponent(selectedStart.name)}&to=${elng},${elat},${encodeURIComponent(selectedEnd.name)}&mode=${travelMode === 'cycling' ? 'bike' : travelMode}`;
    window.open(url, '_blank');
  };

  const handleShareRoute = async () => {
    const text = `从${selectedStart?.name || '起点'}到${selectedEnd?.name || '终点'}的路线：${formatDist(routeResult.distance)}，预计${formatTime(routeResult.duration)}`;
    try {
      ...
      {routeResult && (
        <div className="border border-neutral-100 p-6">
          ...
          <div className="flex flex-wrap gap-3 mt-4 pt-4 border-t border-neutral-100">
            <CTAButton onClick={openInExternalMap} disabled={!selectedStart || !selectedEnd}>
              <ExternalLink size={16} className="inline mr-1" /> 在外部地图中打开
            </CTAButton>
            <CTAButton variant="secondary" onClick={handleShareRoute}>
              {copied ? <Check size={16} className="inline mr-1" /> : <Share2 size={16} className="inline mr-1" />}
              {copied ? '已复制' : '分享路线'}
            </CTAButton>
          </div>
        </div>
