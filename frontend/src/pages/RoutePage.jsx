            ...
            {TRAVEL_MODES.map((m) => {
              ...
              return (
                <button key={m.key} onClick={() => setTravelMode(m.key)}
                  className={`flex items-center gap-1 px-3 py-2 text-xs font-sans uppercase tracking-widest border transition-colors ${travelMode === m.key ? 'border-black text-black' : 'border-neutral-200 text-muted hover:text-black'}`}>
