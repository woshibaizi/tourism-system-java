...
import { chatWithAgent, getAgentSession, getAgentSessions } from '../services/api';
  ...
  return (
    <div className="flex h-[calc(100vh-0px)] md:min-h-0">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-surface">
        ...
        {agentError && (
          <div className="mx-4 mb-2 flex items-center gap-2 px-4 py-2 bg-accent-soft border border-border text-heading text-sm">
            <AlertCircle size={14} />
            <span className="flex-1">{agentError}</span>
            <button onClick={() => setAgentError('')} className="text-accent hover:text-heading">&times;</button>
          </div>
        )}
