...
function DiscoverSlide({ isActive }) {
  ...
  return (
    <div className="flex h-full" style={{ background: '#F7F4EF' }}>
        ...
        <div className="grid grid-cols-3 gap-2 w-full max-w-2xl">
          {PAGE3_PHOTOS.map((src, i) => (
            <motion.div
              ...
              whileHover={{
                scale: 1.08,
                boxShadow: '0 8px 30px rgba(0,0,0,0.45), 0 0 0 1px rgba(155,142,126,0.2)',
                borderColor: 'rgba(155,142,126,0.35)',
                zIndex: 10,
  ...
  return (
    <div className="h-full w-full relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none"
        style={{
          background: 'linear-gradient(160deg, rgba(155,142,126,0.10) 0%, rgba(180,170,155,0.05) 50%, rgba(155,142,126,0.08) 100%)',
      ...
      <div className="absolute inset-y-0 left-0 w-[48%] backdrop-blur-xl bg-black/25 flex items-center">
        <div className="px-10 w-full">
          ...
          <motion.div {...FADE_UP} transition={{ ...FADE_UP.transition, delay: 1.1 }}
            className="mt-8"
            style={{ width: '3rem', height: 0, borderTop: '2px dotted rgba(155,142,126,0.35)' }} />
        </div>
      </div>
    </div>
  );
        ...
        >
          <defs>
            <linearGradient id="mtnBack" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#C4B8A8" stopOpacity="0.18" />
              <stop offset="100%" stopColor="#8A9A8B" stopOpacity="0.08" />
            </linearGradient>
            <linearGradient id="mtnMid" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#C4B8A8" stopOpacity="0.28" />
              <stop offset="100%" stopColor="#8A9A8B" stopOpacity="0.14" />
            </linearGradient>
            <linearGradient id="mtnFront" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#B0A090" stopOpacity="0.38" />
              <stop offset="100%" stopColor="#9B8E7E" stopOpacity="0.22" />
            </linearGradient>
            <filter id="mist">
          ...
          <ellipse cx="300" cy="160" rx="160" ry="18" fill="#C4B8A8" opacity="0.08" filter="url(#mist)" />
          <ellipse cx="1140" cy="160" rx="160" ry="18" fill="#C4B8A8" opacity="0.08" filter="url(#mist)" />
          <path d={MOUNTAIN_MID} fill="url(#mtnMid)" />
          <path d={MOUNTAIN_MID} fill="none" stroke="#8A9A8B" strokeOpacity="0.2" strokeWidth="0.5" />
          <ellipse cx="420" cy="190" rx="180" ry="16" fill="#8A9A8B" opacity="0.07" filter="url(#mist)" />
          <ellipse cx="1020" cy="190" rx="180" ry="16" fill="#8A9A8B" opacity="0.07" filter="url(#mist)" />
          <path d={MOUNTAIN_FRONT} fill="url(#mtnFront)" />
          <path d={MOUNTAIN_FRONT} fill="none" stroke="#9B8E7E" strokeOpacity="0.25" strokeWidth="0.6" />
          <path d="M0,274 Q20,270 40,274 Q60,270 80,274 Q100,270 120,274 Q140,270 160,274 Q180,270 200,274 Q220,270 240,274 Q260,270 280,274 Q300,270 320,274 Q340,270 360,274 Q380,270 400,274 Q420,270 440,274 Q460,270 480,274 Q500,270 520,274 Q540,270 560,274 Q580,270 600,274 Q620,270 640,274 Q660,270 680,274 Q700,270 720,274 Q740,270 760,274 Q780,270 800,274 Q820,270 840,274 Q860,270 880,274 Q900,270 920,274 Q940,270 960,274 Q980,270 1000,274 Q1020,270 1040,274 Q1060,270 1080,274 Q1100,270 1120,274 Q1140,270 1160,274 Q1180,270 1200,274 Q1220,270 1240,274 Q1260,270 1280,274 Q1300,270 1320,274 Q1340,270 1360,274 Q1380,270 1400,274 Q1420,270 1440,274" fill="none" stroke="#B0A090" strokeOpacity="0.15" strokeWidth="0.8" />
        </svg>
      ...
      {pts.map((p, i) => (
        <motion.div
          ...
          style={{
            ...
            background: 'radial-gradient(circle, rgba(155,142,126,0.5) 0%, transparent 70%)',
          }}
          animate={isActive ? {
