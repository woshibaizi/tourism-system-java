...
export const placeAPI = {
  ...
  getPlace: async (placeId) => {
    ...
    const place = normalizePlace(detail.place || {});
    return successResult({
      ...place,
      buildings: detail.buildings || [],
      facilities: detail.facilities || [],
      xhsInfo: detail.xhsInfo || null,
    }, response.message);
  },

  searchPlaces: async (query, type) => {
    const response = await api.get('/places/search', { params: type ? { query, type } : { query } });
    return successResult(normalizeArrayItems(response.data, normalizePlace), response.message);
  },
