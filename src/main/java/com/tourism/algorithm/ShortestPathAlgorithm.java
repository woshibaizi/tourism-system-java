...
public class ShortestPathAlgorithm {
    ...
    /**
     * 启动时从数据库加载图，带重试等待 HikariPool 就绪
     */
    @EventListener(ApplicationReadyEvent.class)
    public void init() {
        loadGraphWithRetry(15, 2000);
    }

    private void loadGraphWithRetry(int maxRetries, long delayMs) {
        for (int i = 0; i < maxRetries; i++) {
            try {
                ...
                System.out.println("[ShortestPathAlgorithm] 图加载成功: " + graph.size() + " 个节点");
                return;
            } catch (Exception e) {
                if (i < maxRetries - 1) {
                    System.err.println("[ShortestPathAlgorithm] 等待数据库就绪 (" + (i + 1) + "/" + maxRetries + ")");
                    try { Thread.sleep(delayMs); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); return; }
                } else {
                    System.err.println("[ShortestPathAlgorithm] 图加载失败: " + e.getMessage());
                }
            }
        }
    }
    ...
    public PathResult tspNearestNeighborDistanceOnly(String start, List<String> destinations) {
        return tspNearestNeighborDistanceOnly(start, destinations, false);
    }

    public PathResult tspNearestNeighborDistanceOnly(String start, List<String> destinations, boolean returnToStart) {
    ...
    public PathResult tspDynamicProgrammingDistanceOnly(String start, List<String> destinations) {
        return tspDynamicProgrammingDistanceOnly(start, destinations, false);
    }

    public PathResult tspDynamicProgrammingDistanceOnly(String start, List<String> destinations, boolean returnToStart) {
        if (destinations == null || destinations.isEmpty()) return new PathResult(List.of(start), 0);
        if (destinations.size() > 15) {
            return tspNearestNeighborDistanceOnly(start, destinations, returnToStart);
        }
        List<String> allNodes = new ArrayList<>();
        allNodes.add(start);
        allNodes.addAll(destinations);
        ...
        int lastNode = -1;
        for (int u = 1; u < n; u++) {
            double d = returnToStart
                    ? matrix.get(allNodes.get(u)).getOrDefault(allNodes.get(0), Double.MAX_VALUE / 2)
                    : 0.0;
    ...
    public PathResult tspSimulatedAnnealingDistanceOnly(String start, List<String> destinations,
                                                         ...
                                                         int maxIterations) {
        return tspSimulatedAnnealingDistanceOnly(start, destinations, initialTemp, coolingRate, maxIterations, false);
    }

    public PathResult tspSimulatedAnnealingDistanceOnly(String start, List<String> destinations,
                                                         double initialTemp, double coolingRate,
                                                         int maxIterations, boolean returnToStart) {
        if (destinations == null || destinations.isEmpty()) return new PathResult(List.of(start), 0);
        List<String> allNodes = new ArrayList<>();
        allNodes.add(start);
        allNodes.addAll(destinations);
        ...
        // 初始解（最近邻）
        PathResult init = tspNearestNeighborDistanceOnly(start, destinations, returnToStart);
    ...
    public PathResult tspGeneticAlgorithmDistanceOnly(String start, List<String> destinations,
                                                       ...
                                                       double mutationRate) {
        return tspGeneticAlgorithmDistanceOnly(start, destinations, populationSize, generations, mutationRate, false);
    }

    public PathResult tspGeneticAlgorithmDistanceOnly(String start, List<String> destinations,
                                                       int populationSize, int generations,
                                                       double mutationRate, boolean returnToStart) {
        if (destinations == null || destinations.isEmpty()) return new PathResult(List.of(start), 0);
        List<String> allNodes = new ArrayList<>();
        allNodes.add(start);
        allNodes.addAll(destinations);
    ...
    public RouteResult multiDestinationRoute(String start, List<String> destinations, String algorithm) {
        return multiDestinationRoute(start, destinations, algorithm, false);
    }

    /**
     * 多目标路径规划（TSP + 详细路径），支持控制是否返回起点
     *
     * @param start         起点
     * @param destinations  目标点列表
     * @param algorithm     算法：nearest_neighbor / dynamic_programming / simulated_annealing / genetic_algorithm
     * @param returnToStart 是否最后返回起点（多点导航场景应为 false）
     * @return 路径规划结果
     */
    public RouteResult multiDestinationRoute(String start, List<String> destinations, String algorithm, boolean returnToStart) {
        ...
        PathResult tspResult = switch (algorithm) {
            case "dynamic_programming" -> tspDynamicProgrammingDistanceOnly(start, reachable, returnToStart);
            case "simulated_annealing" -> tspSimulatedAnnealingDistanceOnly(start, reachable, 1000, 0.995, 5000, returnToStart);
            case "genetic_algorithm" -> tspGeneticAlgorithmDistanceOnly(start, reachable, 100, 500, 0.1, returnToStart);
            default -> tspNearestNeighborDistanceOnly(start, reachable, returnToStart);
        };
