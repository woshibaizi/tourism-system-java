...
public class NavigationController {
    ...
    @Autowired
    private com.tourism.service.NavigationBridgeService navigationBridgeService;
    ...
    /**
     * 多点导航（有序途经点 / 智能优化 / 混合模式）
     */
    @Operation(summary = "多点导航：支持 ordered / optimized / mixed 三种模式")
    @PostMapping("/multi-point")
    public Result<?> multiPointRoute(@RequestBody Map<String, Object> request) {
        String start = (String) request.get("start");
        Double startLat = doubleValue(request.get("startLat"));
        Double startLng = doubleValue(request.get("startLng"));
        @SuppressWarnings("unchecked")
        List<Map<String, Object>> waypointsRaw = (List<Map<String, Object>>) request.get("waypoints");
        String end = (String) request.get("end");
        Double endLat = doubleValue(request.get("endLat"));
        Double endLng = doubleValue(request.get("endLng"));
        String mode = (String) request.getOrDefault("mode", "ordered");
        ...
        if (waypointsRaw == null || waypointsRaw.isEmpty()) {
            return Result.fail("途经点列表(waypoints)不能为空");
        }

        List<OutdoorRouteService.WaypointDef> waypoints = new ArrayList<>();
        for (Map<String, Object> wp : waypointsRaw) {
            String wpNodeId = (String) wp.get("nodeId");
            String wpName = (String) wp.get("name");
            Double wpLat = doubleValue(wp.get("lat"));
            Double wpLng = doubleValue(wp.get("lng"));
            Boolean wpFixed = (Boolean) wp.getOrDefault("fixed", false);
            waypoints.add(new OutdoorRouteService.WaypointDef(wpNodeId, wpName, wpLat, wpLng, wpFixed));
        }

        Map<String, Object> data = outdoorRouteService.planMultiPointRoute(
                start, startLat, startLng, waypoints, end, endLat, endLng,
                mode, algorithm, vehicle, strategy, placeType, provider
        );

        if (data.isEmpty()) {
            return Result.fail("多点导航规划失败，请检查点位是否可达");
        }
        return Result.success(data);
    }
     ...
     * 有序途经点导航
     */
    @Operation(summary = "有序途经点导航：按指定顺序依次经过途经点")
    @PostMapping("/waypoints")
    public Result<?> waypoints(@RequestBody Map<String, Object> request) {
        String start = (String) request.get("start");
        Double startLat = doubleValue(request.get("startLat"));
        Double startLng = doubleValue(request.get("startLng"));
        @SuppressWarnings("unchecked")
        ...
        List<OutdoorRouteService.WaypointDef> waypoints = new ArrayList<>();
        for (Map<String, Object> wp : waypointsRaw) {
            waypoints.add(new OutdoorRouteService.WaypointDef(
                    (String) wp.get("nodeId"),
                    (String) wp.get("name"),
                    doubleValue(wp.get("lat")),
                    doubleValue(wp.get("lng"))
            ));
        }

        Map<String, Object> data = outdoorRouteService.planWaypoints(
                start, startLat, startLng, waypoints, end, endLat, endLng,
                vehicle, strategy, placeType, provider
        );

        if (data.isEmpty()) {
            return Result.fail("途经点导航规划失败，请检查点位是否可达");
        }
        return Result.success(data);
    }
    ...
    public Result<?> multiDestination(@RequestBody Map<String, Object> request) {
        ...
        // 路网图存储 WGS-84，转为 GCJ-02 供前端 AMap 渲染
        data.put("nodeCoordinates", com.tourism.utils.CoordinateTransformUtils.toGcj02NodeCoordinates(
                shortestPathAlgorithm.getNodeCoordinates(result.getPath())));
     ...
     * 室内多房间串联导航
     */
    @Operation(summary = "室内多房间串联导航：按顺序访问多个房间")
    @PostMapping("/indoor/multi-room")
    public Result<?> indoorMultiRoom(@RequestBody Map<String, Object> request) {
        @SuppressWarnings("unchecked")
        ...
        if (roomIds == null || roomIds.size() < 2) {
            return Result.fail("至少需要2个房间ID（roomIds）");
        }
        if (!indoorNavigationAlgorithm.isLoaded()) {
            return Result.fail("室内导航数据未加载，请检查资源文件");
        }

        IndoorNavigationAlgorithm.MultiRoomNavigationResult result =
                indoorNavigationAlgorithm.navigateMultiRoom(roomIds, avoidCongestion, useTimeWeight);
        ...
        Map<String, Object> data = new LinkedHashMap<>();
        data.put("fullPath", result.getFullPath());
        data.put("mergedSteps", result.getMergedSteps());
        data.put("segments", result.getSegments());
        data.put("totalDistance", result.getTotalDistance());
        data.put("totalEstimatedTimeMinutes", result.getTotalEstimatedTime());
        data.put("totalFloorAnalysis", result.getTotalFloorAnalysis());
        data.put("optimizationTarget", result.getOptimizationTarget());
        data.put("roomsVisited", result.getRoomsVisited());
        return Result.success(data);
    }
     ...
     * 室内通用房间到房间导航（支持任意起点）
     */
    @Operation(summary = "室内房间到房间导航（支持任意起点）")
    @PostMapping("/indoor/room-to-room")
    public Result<?> indoorRoomToRoom(@RequestBody Map<String, Object> request) {
        ...
        if (from == null || from.isEmpty() || to == null || to.isEmpty()) {
            return Result.fail("起点(from)和终点(to)不能为空");
        }
        if (!indoorNavigationAlgorithm.isLoaded()) {
            return Result.fail("室内导航数据未加载，请检查资源文件");
        }
        ...
        IndoorNavigationAlgorithm.NavigationResult result =
                indoorNavigationAlgorithm.navigate(from, to, avoidCongestion, useTimeWeight);
     ...
     * 室内外混合导航：室内房间到室外目的地
     */
    @Operation(summary = "室内外混合导航：室内房间 → 室外目的地")
    @PostMapping("/indoor-outdoor")
    public Result<?> indoorToOutdoor(@RequestBody Map<String, Object> request) {
        String roomId = (String) request.get("roomId");
        boolean avoidCongestion = Boolean.TRUE.equals(request.getOrDefault("avoidCongestion", true));
        boolean useTimeWeight = Boolean.TRUE.equals(request.getOrDefault("useTimeWeight", true));
        String outdoorDest = (String) request.get("outdoorDest");
        Double destLat = doubleValue(request.get("destLat"));
        Double destLng = doubleValue(request.get("destLng"));
        String vehicle = (String) request.getOrDefault("vehicle", "步行");
        String strategy = (String) request.getOrDefault("strategy", "time");
        String placeType = (String) request.getOrDefault("placeType", "景区");

        Map<String, Object> data = navigationBridgeService.indoorToOutdoor(
                roomId, avoidCongestion, useTimeWeight,
                outdoorDest, destLat, destLng,
                vehicle, strategy, placeType
        );

        if (data.isEmpty()) return Result.fail("室内外衔接导航失败");
        return Result.success(data);
    }
     ...
     * 室外起点到室内房间导航
     */
    @Operation(summary = "室外起点 → 室内房间导航")
    @PostMapping("/outdoor-indoor")
    public Result<?> outdoorToIndoor(@RequestBody Map<String, Object> request) {
        String outdoorStart = (String) request.get("start");
        Double startLat = doubleValue(request.get("startLat"));
        Double startLng = doubleValue(request.get("startLng"));
        String roomId = (String) request.get("roomId");
        boolean avoidCongestion = Boolean.TRUE.equals(request.getOrDefault("avoidCongestion", true));
        ...
        Map<String, Object> data = navigationBridgeService.outdoorToIndoor(
                outdoorStart, startLat, startLng,
                roomId, avoidCongestion, useTimeWeight,
                vehicle, strategy, placeType
        );

        if (data.isEmpty()) return Result.fail("室外到室内导航失败");
        return Result.success(data);
    }
