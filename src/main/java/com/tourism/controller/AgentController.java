...
public class AgentController {
    ...
    @Operation(summary = "换风格重新生成AI旅行日记（复用图片分析）")
    @PostMapping("/diary/regenerate")
    public Result<Map<String, Object>> regenerateDiary(@AuthenticationPrincipal User principal,
                                                       @RequestBody Map<String, Object> payload) {
        return Result.success(agentService.regenerateDiary(Long.parseLong(principal.getUsername()), payload));
    }

    // ==================== 用户风格偏好 ====================

    @Operation(summary = "获取用户上次使用的风格偏好")
    @GetMapping("/user/style-prefs")
    public Result<Map<String, Object>> getStylePrefs(@AuthenticationPrincipal User principal) {
        return Result.success(agentService.getStylePrefs(Long.parseLong(principal.getUsername())));
    }

    // ==================== 对话式游记生成 & 一键发布 ====================

    @Operation(summary = "从对话历史中生成旅行日记（对话式）")
    @PostMapping("/diary/generate-from-chat")
    public Result<Map<String, Object>> generateDiaryFromChat(@AuthenticationPrincipal User principal,
                                                              @RequestBody Map<String, Object> payload) {
        return Result.success(agentService.generateDiaryFromChat(Long.parseLong(principal.getUsername()), payload));
    }

    @Operation(summary = "一键发布：将对话中生成的游记保存到日记数据库")
    @PostMapping("/diary/publish")
    public Result<Map<String, Object>> publishDiary(@AuthenticationPrincipal User principal,
                                                     @RequestBody Map<String, Object> payload) {
        return Result.success(agentService.publishDiary(Long.parseLong(principal.getUsername()), payload));
    }

    @Operation(summary = "AI润色已有旅行笔记")
    @PostMapping("/diary/polish")
    public Result<Map<String, Object>> polishDiary(@AuthenticationPrincipal User principal,
                                                    @RequestBody Map<String, Object> payload) {
        return Result.success(agentService.polishDiary(Long.parseLong(principal.getUsername()), payload));
    }
