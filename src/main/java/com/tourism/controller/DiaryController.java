...
public class DiaryController {
    ...
    @Operation(summary = "切换点赞（喜欢/取消）")
    @PostMapping("/{id}/like")
    public Result<Map<String, Object>> toggleLike(
            @PathVariable String id,
            @Parameter(description = "用户ID") @RequestParam Long userId) {
        return Result.success(diaryService.toggleLike(id, userId));
    }

    @Operation(summary = "查询用户是否点赞")
    @GetMapping("/{id}/like")
    public Result<Map<String, Object>> getLikeStatus(
            @PathVariable String id,
            @Parameter(description = "用户ID") @RequestParam Long userId) {
        boolean liked = diaryService.hasUserLiked(id, userId);
        Map<String, Object> data = new LinkedHashMap<>();
        data.put("liked", liked);
        return Result.success(data);
    }

    @Operation(summary = "获取用户点赞的日记列表")
    @GetMapping("/liked")
    public Result<List<TravelDiary>> getLikedDiaries(
            @Parameter(description = "用户ID") @RequestParam Long userId) {
        return Result.success(diaryService.getLikedDiaries(userId));
    }

    @Operation(summary = "发布日记到小红书")
    @PostMapping("/{id}/xhs-publish")
    public Result<Map<String, Object>> publishToXhs(
            @PathVariable String id,
            @RequestBody Map<String, Object> payload) {
        Long userId = payload.get("userId") instanceof Number number
                ? number.longValue() : null;
        if (userId == null) {
            return Result.fail(400, "用户ID不能为空");
        }
        return Result.success(diaryService.publishToXhs(id, userId));
    }
}
