...
public interface DiaryService extends IService<TravelDiary> {
     ...
     * 切换点赞（喜欢/取消）
     */
    Map<String, Object> toggleLike(String diaryId, Long userId);

    /**
     * 查询用户是否点赞
     */
    boolean hasUserLiked(String diaryId, Long userId);

    /**
     * 获取用户点赞的日记列表
     */
    List<TravelDiary> getLikedDiaries(Long userId);

    /**
     * 发布日记到小红书
     */
    Map<String, Object> publishToXhs(String diaryId, Long userId);
}
