...
public class AgentClient {
    ...
    public Map<String, Object> regenerateDiary(Map<String, Object> payload) {
        try {
            return agentRestTemplate.postForObject("/agent/diary/regenerate", new HttpEntity<>(payload), Map.class);
        } catch (HttpStatusCodeException exception) {
            throw new BusinessException(exception.getStatusCode().value(), extractErrorMessage(exception));
        } catch (RestClientException exception) {
            throw new BusinessException(503, "个性化旅游助手服务不可用: " + exception.getMessage());
    ...
    public Map<String, Object> getStylePrefs(String userId) {
        return getForMap("/agent/user/style-prefs?user_id={userId}", userId);
    }

    // ==================== 对话式游记生成 & 一键发布 ====================

    public Map<String, Object> generateDiaryFromChat(Map<String, Object> payload) {
        try {
            return agentRestTemplate.postForObject("/agent/diary/generate-from-chat", new HttpEntity<>(payload), Map.class);
        } catch (HttpStatusCodeException exception) {
            throw new BusinessException(exception.getStatusCode().value(), extractErrorMessage(exception));
        } catch (RestClientException exception) {
            throw new BusinessException(503, "个性化旅游助手服务不可用: " + exception.getMessage());
    ...
    public Map<String, Object> publishDiary(Map<String, Object> payload) {
        try {
            return agentRestTemplate.postForObject("/agent/diary/publish", new HttpEntity<>(payload), Map.class);
        } catch (HttpStatusCodeException exception) {
            throw new BusinessException(exception.getStatusCode().value(), extractErrorMessage(exception));
        } catch (RestClientException exception) {
            throw new BusinessException(503, "个性化旅游助手服务不可用: " + exception.getMessage());
    ...
    public Map<String, Object> polishDiary(Map<String, Object> payload) {
        try {
            return agentRestTemplate.postForObject("/agent/diary/polish", new HttpEntity<>(payload), Map.class);
        } catch (HttpStatusCodeException exception) {
            throw new BusinessException(exception.getStatusCode().value(), extractErrorMessage(exception));
        } catch (RestClientException exception) {
            throw new BusinessException(503, "个性化旅游助手服务不可用: " + exception.getMessage());
    ...
    /**
     * 搜索小红书笔记（实时搜索，供 Agent 对话使用）.
     */
    public Map<String, Object> xhsSearchNotes(String query, int num) {
        return getForMap("/agent/xhs/search?query={query}&num={num}", query, num);
    }

    /**
     * 验证小红书 Cookie 是否有效.
     */
    public Map<String, Object> xhsValidateCookie(String cookiesStr) {
        try {
            return agentRestTemplate.postForObject(
                    "/agent/xhs/validate",
                    new HttpEntity<>(Map.of("cookies_str", cookiesStr)),
                    Map.class
            );
        } catch (HttpStatusCodeException exception) {
            throw new BusinessException(exception.getStatusCode().value(), extractErrorMessage(exception));
        } catch (RestClientException exception) {
            throw new BusinessException(503, "小红书验证服务不可用: " + exception.getMessage());
        }
    }
     ...
     * 发布日记到小红书.
     */
    public Map<String, Object> xhsPublishDiary(Map<String, Object> payload) {
        try {
            return agentRestTemplate.postForObject(
                    "/agent/xhs/publish", new HttpEntity<>(payload), Map.class);
        } catch (HttpStatusCodeException exception) {
            throw new BusinessException(exception.getStatusCode().value(), extractErrorMessage(exception));
        } catch (RestClientException exception) {
            throw new BusinessException(503, "小红书发布服务不可用: " + exception.getMessage());
        }
    }
     ...
     * 按需刷新景点的小红书热度数据（全量抓取）.
     */
    public Map<String, Object> xhsRefreshPlace(String placeName, String placeId, String placeType) {
        try {
            String url = "/agent/xhs/refresh-place?place_name={placeName}&place_id={placeId}&place_type={placeType}";
            return agentRestTemplate.postForObject(url, null, Map.class, placeName, placeId, placeType);
        } catch (HttpStatusCodeException exception) {
            throw new BusinessException(exception.getStatusCode().value(), extractErrorMessage(exception));
        } catch (RestClientException exception) {
            throw new BusinessException(503, "小红书抓取服务不可用: " + exception.getMessage());
        }
    }
