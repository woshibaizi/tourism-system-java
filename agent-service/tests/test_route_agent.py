...
class TestToolsRegistered:
    ...
    def test_route_tools_have_proper_schemas(self):
        ...
        # recommend_places 有 interests 参数（非必填，不传则默认通用兴趣）
        rec = tools.get("recommend_places")
        assert rec is not None
        assert "interests" in rec["function"]["parameters"].get("properties", {})
        # recommend_places 的 required 为空（interests 可选）
        assert rec["function"]["parameters"].get("required", ["dummy"]) == []
