...
class TourismApiClient:
    ...
    def search_surroundings(
        self, query: str, place_id: str | None = None
    ) -> list[dict[str, Any]]:
        """搜索校园/景区周边的商户和服务设施。"""
        ...
        try:
            payload = self._get_json(f"/api/surroundings/search?{query_str}")
        except URLError:
            return []
        data = payload.get("data")
        if isinstance(data, list):
    ...
    def get_surroundings_by_place(
        self, place_id: str, stype: str | None = None
    ) -> list[dict[str, Any]]:
        """获取指定场所的周边商户列表，可选按类型过滤。"""
        path = f"/api/surroundings/place/{place_id}"
        if stype:
            path += f"/type/{stype}"
        try:
    ...
    def get_surrounding_detail(self, s_id: str) -> dict[str, Any]:
        """获取周边商户详情。"""
        try:
            payload = self._get_json(f"/api/surroundings/{s_id}")
        except URLError:
            return {}
        return payload.get("data", {})

    def get_surrounding_categories(self, place_id: str) -> dict[str, Any]:
        """获取某场所的周边分类统计。"""
        try:
            payload = self._get_json(f"/api/surroundings/categories/{place_id}")
        except URLError:
            return {}
        return payload.get("data", {})

    def get_hot_surroundings(self, size: int = 10) -> list[dict[str, Any]]:
        """获取热门周边商户。"""
        query = urlencode({"limit": size})
        try:
            payload = self._get_json(f"/api/surroundings/hot?{query}")
        except URLError:
            return []
        data = payload.get("data")
        if isinstance(data, list):
